                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative flex -space-x-2">
                        {contacts.slice(5, 8).map((contact, index) => (
                          <div 
                            key={contact.email}
                            className="w-6 h-6 rounded-full bg-[#1E1E3F] flex items-center justify-center text-white text-xs ring-2 ring-white"
                          >
                            {contact.name.charAt(0)}
                          </div>
                        ))}
            ...
            <AnimatePresence>
              {showSuccess && (
                ...
                >
                  ...
                  Export complete! Your file is ready.
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <AnimatePresence>
              {showShareLink && (
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.95 }}
                  className="bg-blue-50 text-blue-800 rounded-xl p-4 flex items-center gap-3"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
